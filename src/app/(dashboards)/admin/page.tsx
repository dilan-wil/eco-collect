...
import { agentsApi, vehiculesApi } from "@/lib/api";
      ...
      <Card className="overflow-hidden relative">
        ...
        <CardContent className="p-5 relative z-10">
          <div className="flex items-start justify-between mb-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center ${color} bg-opacity-15`}
            >
              <Icon className="w-5 h-5 text-white" />
            </div>
            {trend !== undefined && (
              <span
                className={`text-xs font-semibold flex items-center gap-0.5 ${up ? "text-green-600" : "text-red-500"}`}
              >
                {up ? (
...
function AgentStatusDot({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-block w-2 h-2 rounded-full ${cls} ${status === "En mission" ? "animate-pulse" : ""}`}
    />
  );
}
...
export default function AdminDashboard() {
  ...
  const urgent = signalements.filter(
    (r) =>
      r.priorite === "critique" &&
      r.statut !== "resolu" &&
      r.statut !== "ferme" &&
  ...
  const calculatePercentageWaste = (number: number) => {
    return number/signalements.length * 100
  }

  const radialData = [
    { name: "Taux", value: calculatePercentageWaste(signalements.filter(s => s.statut==="resolu").length), fill: "#16A34A" },
  ];

  const wasteTypes = [
    { name: "Ménager", value: calculatePercentageWaste(signalements.filter(s => s.categorie==="Ménager").length) },
    { name: "Plastique", value: calculatePercentageWaste(signalements.filter(s => s.categorie==="Plastique").length) },
    { name: "Construction", value: calculatePercentageWaste(signalements.filter(s => s.categorie==="Construction").length) },
    { name: "Organique", value: calculatePercentageWaste(signalements.filter(s => s.categorie==="Organique").length) },
    { name: "Électronique", value: calculatePercentageWaste(signalements.filter(s => s.categorie==="Électronique").length) },
    { name: "Dangereux", value: calculatePercentageWaste(signalements.filter(s => s.categorie==="Dangereux").length) },
  ]
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard
          ...
          value={agents.filter(agent => agent.statut==="disponible").length}
          sub={`sur ${agents.length} disponibles`}
          color="bg-blue-600"
          icon={Users}
          delay={0.05}
        />
        <MetricCard
          label="Validation IA"
          value={`${mockStats.aiValidationRate}%`}
          trend={2.1}
          color="bg-violet-600"
          icon={BrainCircuit}
          delay={0.1}
                ...
                <div className="relative h-[150px] w-[150px]">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-3xl font-black">
                      {calculatePercentageWaste(signalements.filter(s => s.statut==="resolu").length)}%
                    </span>
          ...
          {/* <Card className="shadow-sm h-full">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-bold flex items-center gap-2">
                <MapPin className="w-4 h-4 text-primary" /> Points noirs
                (Arrondissements)
              </CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <Link key={r.id} href={`/admin/signalements/${r.id}`}>
                  <div className="p-3 rounded-lg border border-destructive/20 bg-destructive/5 hover:bg-destructive/10 transition-colors cursor-pointer">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-destructive text-white shrink-0">
                        {r.priorite}
                      </span>
                    </div>
            ...
            <CardContent className="space-y-3">
              {vehicules.slice(0, 4).map((v) => {
                ...
                return (
                  <div
                    key={v.id}
                    className="p-2.5 rounded-lg border border-border hover:bg-muted/40 transition-colors"
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span
                        className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${statusBg}`}
                      >
                        {v.statut}
                      </span>
                    </div>
          ...
          <Card className="shadow-sm overflow-hidden flex flex-col">
            <div className="bg-gradient-to-br from-violet-600 to-indigo-600 p-5">
              ...
              <p className="text-violet-200 text-xs">
                3 actions prioritaires détectées
              </p>
            </div>
            <CardContent className="p-0 flex-1">
              <div className="divide-y text-sm">
                {[
                  {
                    icon: TrendingUp,
                    text: "Renforcer la collecte 3ème Arr.",
                    sub: "Pic +15% depuis 48h",
                    color: "text-orange-500",
                  },
                  {
                    icon: Truck,
                    text: "Maintenance préventive VEH-002",
                    sub: "Prochaine révision dans 3 jours",
                    color: "text-blue-500",
                  },
                  {
                    icon: Zap,
                    text: "Optimiser tournée Sud-Ouest",
                    sub: "Économie estimée : 2,4h agent/jour",
