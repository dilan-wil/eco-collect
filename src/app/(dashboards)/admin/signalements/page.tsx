...
export default function SignalementsAdmin() {
  ...
  const filteredReports = signalements.filter(r => {
    ...
    if (mapped && mapped !== '__priority__' && r.statut !== mapped) return false
    if (search && !r.id.toLowerCase().includes(search.toLowerCase()) && !r?.adresse?.toLowerCase().includes(search.toLowerCase())) return false
    return true
  })
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div className="mb-6 sm:mb-8">
        ...
        <p className="text-muted-foreground mt-1 text-sm sm:text-base">
          {filteredReports.length} dépôt{filteredReports.length > 1 ? 's' : ''} sauvage{filteredReports.length > 1 ? 's' : ''} signalé{filteredReports.length > 1 ? 's' : ''}
        </p>
      </div>
      ...
      <div className="bg-card border rounded-xl p-3 sm:p-4 mb-4 sm:mb-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2">
          {['Tous', 'Nouveaux', 'En Cours', 'Resolus', 'Fermés', 'Réjetés'].map(f => (
            <button
              key={f}
            ...
            <tbody className="divide-y divide-border">
              {filteredReports.map((report) => (
                <tr key={report.id} className="hover:bg-muted/30 transition-colors group">
                  <td className="px-4 py-4 font-mono text-xs">{report.id.slice(0, 8)}</td>
                  ...
                  <td className="px-4 py-4">
                    <p className="font-medium text-foreground max-w-[180px] truncate">{report.adresse}</p>
                    <p className="text-xs text-muted-foreground">{report.ville}</p>
                  </td>
                  <td className="px-4 py-4">
                    <p className="font-medium text-sm">{report.categorie}</p>
                    <p className="text-xs text-muted-foreground">{report.niveau_accumulation}</p>
                  </td>
                  <td className="px-4 py-4">
                    ...
                    {report.confiance_ia > 0 ? (
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-1 text-xs font-medium">
                          <span className={`w-2 h-2 rounded-full ${report.confiance_ia > 90 ? 'bg-green-500' : 'bg-amber-500'}`}></span>
                          {report.confiance_ia}%
                        </div>
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      {report.statut === 'nouveau' && (
                        <Button size="sm" variant="default" className="h-8 text-xs hidden 2xl:inline-flex">
                          Assigner
                        </Button>
                      )}
      ...
      <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        {filteredReports.map((report) => (
          <Link 
            ...
            className="block"
          >
            <div className="bg-card border rounded-xl p-4 hover:shadow-md transition-shadow h-full">
              {/* Header */}
              ...
              <div className="flex items-start gap-2 mb-2">
                ...
                <div>
                  <p className="text-sm font-medium line-clamp-2">{report.adresse}</p>
                  <p className="text-xs text-muted-foreground">{report.ville}</p>
                </div>
              </div>
              ...
              {report.confiance_ia > 0 && (
                <div className="flex items-center gap-2 mt-2 pt-2 border-t">
                  <div className="flex items-center gap-1.5">
                    <span className={`w-1.5 h-1.5 rounded-full ${report.confiance_ia > 90 ? 'bg-green-500' : 'bg-amber-500'}`}></span>
                    <span className="text-xs font-medium">{report.confiance_ia}%</span>
