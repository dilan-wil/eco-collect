...
import {
  PlusCircle, MapPin, Star, TrendingUp, CheckCircle2,
  Clock, Leaf, ChevronRight, Camera, Zap, Award, AlertCircle
} from "lucide-react"

const myReports = mockReports.filter(r => r.citizenId === "CIT-001")
const pending   = myReports.filter(r => r.status === 'En attente').length
const active    = myReports.filter(r => ['Validé','Assigné','En cours'].includes(r.status)).length
...
const wasteIcon: Record<string, string> = {
  Plastique: '♻️', Ménager: '🗑️', Organique: '🌱',
  Construction: '🏗️', Électronique: '💻', Dangereux: '⚠️',
}
...
function ReportRow({ r, i }: { r: typeof myReports[0]; i: number }) {
  return (
    <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 * i }}>
    ...
    <>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          ...
          <h1 className="text-3xl font-black tracking-tight">Jean Dupont</h1>
        </div>
          ...
          <div className="bg-gradient-to-r from-primary to-secondary rounded-2xl p-5 flex items-center gap-4 mb-8 cursor-pointer hover:opacity-95 transition-opacity shadow-lg shadow-primary/20 group">
            ...
            <div className="flex-1 text-white">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="bg-white/20 text-xs px-2 py-0.5 rounded-full font-medium ml-1">{USER_LEVEL}</span>
              </div>
              <div className="w-full bg-white/20 rounded-full h-1.5 mt-2">
                <div className="bg-white rounded-full h-1.5 w-[70%]" />
              </div>
              <p className="text-white/70 text-xs mt-1">180 pts pour le prochain niveau</p>
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard value={myReports.length} label="Total signalements"   icon={Camera}       color="bg-primary/10 text-primary"          delay={0.1} />
        <StatCard value={pending}          label="En attente"            icon={Clock}        color="bg-amber-100 text-amber-600"         delay={0.15} />
        <StatCard value={active}           label="En traitement"         icon={Zap}          color="bg-blue-100 text-blue-600"           delay={0.2} />
        <StatCard value={completed}        label="Collectés"             icon={CheckCircle2} color="bg-green-100 text-green-600"         delay={0.25} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-card border rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-2">
              {myReports.slice(0, 5).map((r, i) => <ReportRow key={r.id} r={r} i={i} />)}
            </div>
          </div>
          ...
          <div className="bg-card border rounded-2xl shadow-sm p-5">
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              {[
                { value: '45 kg',   label: 'Plastique recyclé',   color: 'bg-blue-50 text-blue-700' },
                { value: '120 kg',  label: 'Déchets organiques',  color: 'bg-green-50 text-green-700' },
                { value: '2 m²',    label: 'Espace libéré',       color: 'bg-violet-50 text-violet-700' },
              ].map(s => (
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={b.label}>
                  <div className="flex justify-between text-xs mb-1 text-muted-foreground"><span>{b.label}</span><span className="font-medium">{b.pct}%</span></div>
                  <div className="w-full bg-muted rounded-full h-2">
                    <motion.div className={`h-2 rounded-full ${b.color}`}
                      initial={{ width: 0 }} animate={{ width: `${b.pct}%` }} transition={{ duration: 1, delay: 0.4 }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <motion.div whileHover={{ y: -3 }}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm">Vos signalements ont permis de collecter <strong>12 dépôts</strong> ce mois-ci. Continuez comme ça !</p>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card border rounded-2xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              ...
              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-green-500 inline-block"/>Collecté</span>
          ...
          <div className="bg-card border rounded-2xl shadow-sm p-5">
            <h2 className="font-bold text-base mb-4 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-primary" /> Conseils pour gagner plus de points
            </h2>
            <div className="space-y-3">
              {[
                { icon: Camera,       tip: 'Photo nette = +20 pts bonus',           done: true },
                { icon: AlertCircle,  tip: 'Indiquer la priorité correcte',          done: true },
                { icon: MapPin,       tip: 'Localisation précise aide les agents',   done: false },
                { icon: Zap,          tip: 'Signalez rapidement = +10 pts vitesse',  done: false },
              ].map((t, i) => (
                <div key={i} className={`flex items-center gap-3 text-sm p-2 rounded-lg ${t.done ? 'opacity-50' : ''}`}>
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${t.done ? 'bg-primary/10' : 'bg-amber-50'}`}>
                    <t.icon className={`w-3.5 h-3.5 ${t.done ? 'text-primary' : 'text-amber-600'}`} />
                  </div>
                  <span className={t.done ? 'line-through text-muted-foreground' : 'font-medium'}>{t.tip}</span>
                  {t.done && <CheckCircle2 className="w-3.5 h-3.5 text-primary ml-auto shrink-0" />}
                </div>
              ))}
            </div>
          </div>
