...
import {
  Leaf, ArrowRight, BrainCircuit, Truck, Users,
  MapPin, Zap, ShieldCheck, BarChart3, ChevronRight,
  Circle, CheckCircle2, Star, Clock
} from "lucide-react"
...
const FEED = [
  ...
  { type: "Construction", arr: "1er",   ago: "il y a 5 min",  color: "bg-amber-500" },
  { type: "Organique",    arr: "5ème",  ago: "il y a 8 min",  color: "bg-green-500" },
  { type: "Électronique", arr: "3ème",  ago: "il y a 12 min", color: "bg-violet-500" },
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <span className="text-white/70 text-sm">
          Nouveau signalement <span className="text-white font-semibold">{item.type}</span>
          {" "}· Arrondissement {item.arr}
        </span>
        <span className="text-white/40 text-xs ml-auto shrink-0">{item.ago}</span>
      </motion.div>
    </AnimatePresence>
  )
}
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {[
        ...
        <motion.div
          ...
          style={{ width: o.w, height: o.w, left: o.x, top: o.y, backgroundColor: o.c, opacity: 0.18 }}
...
// ─── Role card ────────────────────────────────────────────────────────────────
function RoleCard({
  role, label, desc, href, gradient, icon: Icon, features, delay
}: {
  role: 'citoyen' | 'admin' | 'agent'
  label: string; desc: string; href: string; gradient: string
  icon: React.ElementType; features: string[]; delay: number
}) {
...
function StepCard({ n, title, desc, icon: Icon, color }: {
  ...
  return (
    ...
    >
      <div className={`shrink-0 w-14 h-14 rounded-2xl ${color} flex items-center justify-center shadow-lg`}>
        <Icon className="w-7 h-7 text-white" />
      </div>
      <div>
        <span className="text-xs font-black uppercase tracking-[0.15em] text-white/30 mb-1 block">Étape {n}</span>
      ...
      <header className="fixed top-0 w-full z-50 border-b border-white/8 backdrop-blur-xl bg-[#080F0A]/80">
        <div className="max-w-7xl mx-auto px-6 md:px-10 h-18 flex items-center justify-between py-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-black text-xl tracking-tight">EcoCollect<span className="text-primary">.ai</span></span>
          </div>

          {/* Desktop nav */}
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
            {[['#fonctionnalites', 'Fonctionnalités'], ['#comment', 'Comment ça marche'], ['#impact', 'Impact']].map(([h, l]) => (
              <a key={h} href={h} className="text-white/50 hover:text-white transition-colors">{l}</a>
            ))}
          </nav>

          <div className="flex items-center gap-3">
            <Link href="/login">
              <Button variant="ghost" className="hidden md:flex text-white/70 hover:text-white hover:bg-white/10 rounded-full px-5">
                Connexion
              </Button>
            </Link>
            <Link href="/register">
      ...
      <main className="flex-1">
        ...
        <section className="relative min-h-screen flex flex-col justify-center pt-18 overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-10 pt-20 pb-12">
            ...
            >
              ...
              <span className="relative inline-block">
                <span
                  ...
                  style={{ backgroundImage: 'linear-gradient(135deg, #16A34A 0%, #4ADE80 40%, #2563EB 100%)' }}
                >
                  commence par vous.
                </span>
                ...
                <motion.svg
                  ...
                  viewBox="0 0 400 14" fill="none"
                  preserveAspectRatio="none"
                >
                  <motion.path
                    d="M2 10 Q100 2 200 8 Q300 14 398 6"
                    stroke="url(#uline)" strokeWidth="3" strokeLinecap="round"
                    initial={{ pathLength: 0 }} animate={{ pathLength: 1 }}
                    transition={{ duration: 0.9, delay: 0.9, ease: "easeOut" }}
                  />
                  <defs>
                    <linearGradient id="uline" x1="0" y1="0" x2="400" y2="0" gradientUnits="userSpaceOnUse">
            ...
            >
              EcoCollect AI connecte les citoyens, les agents de collecte et les administrateurs dans une plateforme unifiée, propulsée par l'intelligence artificielle.
            </motion.p>
            ...
            >
              <Link href="/citoyen/dashboard" onClick={() => setRole('citoyen')}>
                <Button size="lg" className="rounded-full h-14 px-8 text-base bg-primary hover:bg-primary/90 shadow-2xl shadow-primary/30">
                  Faire un signalement
                  <ChevronRight className="w-5 h-5 ml-1" />
                </Button>
              </Link>
              <Link href="/admin/dashboard" onClick={() => setRole('admin')}>
                <Button size="lg" variant="outline" className="rounded-full h-14 px-8 text-base border-white/20 bg-white/5 text-white hover:bg-white/10">
            ...
            >
              {[
                ...
                <div key={i} className="flex flex-col">
                  ...
                  <span className="text-xs text-white/40 font-medium uppercase tracking-wider mt-0.5">{s.label}</span>
                </div>
              ))}
            </motion.div>
            ...
            >
              <div className="flex items-center gap-2 mb-2.5">
                ...
                <span className="text-[11px] font-bold text-white/30 uppercase tracking-widest">Activité en direct</span>
              </div>
              <LiveTicker />
            </motion.div>
          </div>
        ...
        {/* ── ROLES ── */}
        <section id="fonctionnalites" className="py-32 px-6 md:px-10 bg-[#080F0A]">
          <div className="max-w-7xl mx-auto">
            <motion.div
              ...
              className="mb-16"
            >
              <span className="text-xs font-black uppercase tracking-[0.2em] text-primary mb-3 block">Trois rôles. Un écosystème.</span>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <RoleCard
                role="citoyen" label="Citoyen" href="/citoyen/dashboard"
                ...
                icon={Users}
                features={["Signalement photo en 4 étapes", "Analyse IA instantanée", "Points & badges de récompense", "Suivi en temps réel"]}
                delay={0}
              />
              <RoleCard
                role="admin" label="Administrateur" href="/admin/dashboard"
                desc="Supervisez l'ensemble des opérations, gérez les agents et visualisez les analytics de la ville."
                gradient="bg-gradient-to-br from-blue-600 to-indigo-600"
                icon={BarChart3}
                features={["Mission Control centralisé", "Gestion agents & véhicules", "Analytiques avancées", "Recommandations IA"]}
                delay={0.1}
              />
              <RoleCard
                role="agent" label="Agent de Collecte" href="/agent/dashboard"
                desc="Accédez à votre tournée optimisée, enregistrez vos interventions et suivez vos performances."
                gradient="bg-gradient-to-br from-violet-600 to-purple-700"
                icon={Truck}
                features={["Tournée GPS optimisée", "Carte interactive", "Validation photo", "Score de performance"]}
                delay={0.2}
              />
            </div>
          </div>
        ...
        {/* ── HOW IT WORKS ── */}
        <section id="comment" className="py-32 px-6 md:px-10 relative overflow-hidden">
          {/* Background */}
            ...
            <div className="grid lg:grid-cols-2 gap-20 items-center">
              ...
              <div>
                <motion.div
                  ...
                  className="mb-14"
                >
                  <span className="text-xs font-black uppercase tracking-[0.2em] text-primary mb-3 block">Le processus</span>
                  <h2 className="text-4xl md:text-5xl font-black text-white leading-tight">
                    De la rue à la solution <br />en moins de 48h
                  </h2>
                </motion.div>

                <div className="space-y-10">
                  <StepCard n="01" title="Signalement citoyen"
                    desc="Un citoyen photographie un dépôt sauvage. L'app détecte automatiquement la position GPS et prépare l'analyse."
                    icon={MapPin} color="bg-emerald-600" />
                  <StepCard n="02" title="Analyse par l'IA"
                    ...
                    icon={BrainCircuit} color="bg-blue-600" />
                  <StepCard n="03" title="Mission assignée"
                  ...
                  <StepCard n="04" title="Résolution & récompense"
              ...
              >
                <div className="relative">
                    ...
                    <div className="p-6 space-y-5">
                      ...
                      <div className="rounded-2xl bg-gradient-to-br from-blue-600/20 to-violet-600/10 border border-blue-500/20 p-5">
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <span className="text-blue-400 font-bold text-sm">Analyse IA complète</span>
                          <span className="ml-auto text-xs text-white/40">0.8s</span>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          {[
                            { label: "Type détecté", value: "Plastique", color: "text-blue-300" },
                            { label: "Confiance", value: "94%", color: "text-green-400" },
                            { label: "Volume estimé", value: "Moyen", color: "text-amber-300" },
                            { label: "Priorité", value: "Haute", color: "text-orange-400" },
                          ].map((item, i) => (
                      ...
                      <div className="space-y-3">
                        {[
                          ...
                          { label: "Collecte en cours", done: false, active: true },
                        ].map((s, i) => (
                          <div key={i} className="flex items-center gap-3">
                            ...
                            <span className={`text-sm ${s.done ? 'text-white/70 line-through' : s.active ? 'text-white font-semibold' : 'text-white/30'}`}>{s.label}</span>
                            {s.active && <span className="ml-auto text-[10px] font-bold text-blue-400 animate-pulse">EN COURS</span>}
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="rounded-2xl bg-gradient-to-r from-primary/20 to-emerald-500/10 border border-primary/20 p-4 flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-white text-sm font-bold">+50 points gagnés !</p>
                          <p className="text-white/50 text-xs">Signalement validé · Niveau 3 → 4</p>
                        </div>
                        <div className="ml-auto text-2xl font-black text-primary">🎉</div>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <p className="text-white/70 text-xs font-medium">validé par IA</p>
                  </motion.div>
        ...
        {/* ── IMPACT ── */}
        <section id="impact" className="py-32 px-6 md:px-10 bg-[#080F0A] relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto">
            ...
            >
              <span className="text-xs font-black uppercase tracking-[0.2em] text-primary mb-3 block">Impact réel</span>
              <h2 className="text-4xl md:text-5xl font-black text-white">
                Lyon plus propre, chaque jour
              </h2>
            </motion.div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/8 rounded-3xl overflow-hidden">
              {[
                { n: 12450, s: '+', label: "Signalements", sub: "depuis le lancement", color: "text-emerald-400" },
                { n: 87,    s: '%', label: "IA Accuracy",  sub: "taux de validation",  color: "text-blue-400" },
                { n: 93,    s: '%', label: "Succès",       sub: "missions complétées", color: "text-violet-400" },
                { n: 420,   s: '',  label: "Citoyens",     sub: "actifs cette semaine", color: "text-amber-400" },
              ].map((s, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 16 }}
                ...
                >
                  <p className={`text-5xl md:text-6xl font-black mb-2 ${s.color}`}>
                    <Counter to={s.n} suffix={s.s} />
                  </p>
            ...
            <div className="mt-16 flex flex-wrap justify-center gap-3">
              {[
                ...
                { icon: MapPin,      text: "Couverture Lyon Métropole" },
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 p-10 md:p-14">
                <div>
                  ...
                  <p className="text-white/80 text-lg">
                    Rejoignez les 420+ citoyens qui contribuent chaque jour à Lyon.
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 shrink-0">
                  <Link href="/register">
                    <Button size="lg" className="rounded-full h-14 px-8 bg-white text-primary hover:bg-white/90 font-bold shadow-xl text-base">
                      Créer un compte gratuit
                    </Button>
                  </Link>
                  <Link href="/login">
                    <Button size="lg" variant="ghost" className="rounded-full h-14 px-8 text-white border border-white/30 hover:bg-white/10 text-base">
                      Se connecter
                    </Button>
                  </Link>
                </div>
      ...
      <footer className="border-t border-white/8 py-14 px-6 md:px-10">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start gap-10 mb-12">
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="font-black text-xl">EcoCollect<span className="text-primary">.ai</span></span>
              </div>
              <p className="text-white/40 text-sm max-w-xs leading-relaxed">
                Plateforme SaaS de gestion intelligente des déchets urbains. Prototype de démonstration.
              </p>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 gap-10 text-sm">
              <div>
                <p className="text-white/30 font-bold uppercase tracking-wider text-xs mb-4">Portails</p>
                <div className="space-y-2.5">
                  <Link href="/citoyen/dashboard" onClick={() => setRole('citoyen')} className="block text-white/60 hover:text-white transition-colors">Espace Citoyen</Link>
                  <Link href="/admin/dashboard" onClick={() => setRole('admin')} className="block text-white/60 hover:text-white transition-colors">Mission Control</Link>
                  <Link href="/agent/dashboard" onClick={() => setRole('agent')} className="block text-white/60 hover:text-white transition-colors">Espace Agent</Link>
                </div>
              </div>
              <div>
                <p className="text-white/30 font-bold uppercase tracking-wider text-xs mb-4">Compte</p>
                <div className="space-y-2.5">
                  <Link href="/login" className="block text-white/60 hover:text-white transition-colors">Connexion</Link>
                  <Link href="/register" className="block text-white/60 hover:text-white transition-colors">Inscription</Link>
                </div>
              </div>
              <div>
                <p className="text-white/30 font-bold uppercase tracking-wider text-xs mb-4">Projet</p>
                <div className="space-y-2.5">
                  <span className="block text-white/60">Lyon, France</span>
                  <span className="block text-white/60">v1.0 · Démo</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-white/8 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-white/25">
            <span>© 2024 EcoCollect AI. Tous droits réservés.</span>
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
              <span>Toutes les données sont fictives · Prototype</span>
            </div>
          </div>
        </div>
      </footer>
